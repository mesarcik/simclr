...
def get_augmented_data(data_name, size=32, s=1.0):
    k_size= int(0.1*size)
    print(k_size)
    ...
    if data_name == 'CIFAR10':
        ...
        trainloader_t0 = DataLoader(trainset_t0, batch_size=_batch_size, shuffle=False, num_workers=2)

        trainset_t1 = datasets.CIFAR10(root='/home/mmesarcik/data', train=True, download=True, transform=data_transforms)
        trainloader_t1 = DataLoader(trainset_t1, batch_size=_batch_size, shuffle=False, num_workers=2)

        testset = datasets.CIFAR10(root='/home/mmesarcik/data', train=False, download=True, transform=data_transforms)
        testloader = DataLoader(testset, batch_size=_batch_size,shuffle=False, num_workers=2)
